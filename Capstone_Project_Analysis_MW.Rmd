...
After inspecting the deltas, I realized that the price deltas were misleading since deltas for high cost values were more significant than deltas for low cost values  since the consumer will value saving $1 for a $2 item as much as saving $10 for $100 item, I decided to create another field, p difference to weigh high price deltas equal to low price deltas.  
...
The histograms for all the categories do not appear to follow a normal distribution.  In addition, each category has some extreme outliers. I performed a quick Anderson-Darling Normality test just to validate.  
